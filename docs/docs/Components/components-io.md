...
# Input and output components in CentaurOps
...
4. In the CentaurOps UI, click your flow name, and then click **Logs**.
The **Logs** pane opens.
...
The **Chat Input** component is often the entry point for passing messages to the CentaurOps API.
To send the same example messages programmatically to your CentaurOps server, do the following:

1. To get your CentaurOps endpoint, click **Publish**, and then click **API access**.
...
```
The custom `session_id` value starts a new chat session between your client and the CentaurOps server, and can be useful in keeping conversations and AI context separate.
