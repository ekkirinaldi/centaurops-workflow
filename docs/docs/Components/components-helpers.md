...
# Helper components in CentaurOps
...
Chat memory in CentaurOps is stored either in local CentaurOps tables with `LCBufferMemory`, or connected to an external database.
...
The Current Date component returns the current date and time in a selected timezone. This component provides a flexible way to obtain timezone-specific date and time information within a CentaurOps pipeline.
...
:::info
Prior to CentaurOps 1.1, this component was known as the Chat Memory component.
:::

This component retrieves chat messages from CentaurOps tables or external memory.

In this example, the **Message Store** component stores the complete chat history in a local CentaurOps table, which the **Message History** component retrieves as context for the LLM to answer each question.

![Message store and history components](/img/component-message-history-message-store.png)

For more information on configuring memory in CentaurOps, see [Memory](/memory).
...
| Name | Type | Description |
|------|------|-------------|
| memory | Memory | Retrieve messages from an external memory. If empty, the CentaurOps tables are used. |
...
This component stores chat messages or text in CentaurOps tables or external memory.
...
| memory | Memory | The external memory to store the message. If empty, the CentaurOps tables are used. |
