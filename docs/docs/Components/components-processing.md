...
Replace `YOUR_FLOW_ID` with your flow ID.
This example uses the default CentaurOps server address.
```text
curl -X POST "http://127.0.0.1:7860/api/v1/webhook/YOUR_FLOW_ID" \
-H 'Content-Type: application/json' \
-d '{
...
This example uses `./output/employees.xlsx` and `./output/employees.md` to save the files in a directory relative to where CentaurOps is running.
...
:::important
This component is in **Legacy**, which means it is available for use but no longer in active development.
Prior to CentaurOps version 1.1.3, this component was named **Merge Data**.
:::
