...
| Is Metadata | When loading data using LangChain or CentaurOps, additional attributes may be stored under a metadata object. If the target attribute is stored this way, enable this option. It adjusts the query by generating a filter in the format: `{"metadata.<attribute_name>": "<value>"}` |
...
The **MCP connection** component connects to a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) server and exposes the MCP server's tools as tools for CentaurOps agents.

In addition to being an MCP client that can leverage MCP servers, the **MCP connection** component's [SSE mode](#mcp-sse-mode) allows you to connect your flow to the CentaurOps MCP server at the `/api/v1/mcp/sse` API endpoint, exposing all flows within your [project](/concepts-overview#projects) as tools within a flow.
...
2. In the **MCP connection** component, in the **MCP Command** field, enter the command to start your MCP server. For example, to start a [Fetch](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch) server, the command is:
    ...
    `uvx` is included with `uv` in the CentaurOps package.
    ...
    For an example of starting `npx` MCP servers, see [Connect an Astra DB MCP server to CentaurOps](/mcp-component-astra).
    ...
    :::important
    CentaurOps passes environment variables from the `.env` file to MCP, but not global variables declared in the UI.
    To add a value for an environment variable as a global variable, add it to CentaurOps's `.env` file at startup.
...
This confirms the MCP server is connected, and its tools are being used in CentaurOps.
...
:::important
If you're using **CentaurOps for Desktop**, the default address is `http://127.0.0.1:7868/`.
:::

The MCP component's SSE mode connects your flow to the CentaurOps MCP server through the component.
...
2. In the **MCP SSE URL** field, modify the default address to point at the SSE endpoint of the CentaurOps server you're currently running.
...
### MCP Tools (stdio)
:::important
This component is deprecated as of CentaurOps version 1.3.
