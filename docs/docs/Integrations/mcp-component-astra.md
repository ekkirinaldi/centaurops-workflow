---
title: Connect an Astra DB MCP server to CentaurOps
...
Use the [MCP connection component](/components-tools#mcp-connection) to connect CentaurOps to a [Datastax Astra DB MCP server](https://github.com/datastax/astra-db-mcp).
...
8. In the **MCP connection** component, in the **Env** fields, add variables for `ASTRA_DB_APPLICATION_TOKEN` and `ASTRA_DB_API_ENDPOINT` with the values from your Astra database.
    ...
    To add the values for `ASTRA_DB_APPLICATION_TOKEN` and `ASTRA_DB_API_ENDPOINT` as global variables, add them to CentaurOps's `.env` file at startup.
...
11. Open the **Playground**, and then ask the agent, `What collections are available?`
    ...
    Since CentaurOps is connected to your Astra DB database through the MCP, the agent chooses the correct tool and connects to your database to retrieve the answer.
