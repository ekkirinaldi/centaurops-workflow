...
The CentaurOps command line interface (CentaurOps CLI) is the main interface for managing and running the CentaurOps server.
...
Copy the CentaurOps database files, `langflow.db` and `langflow-pre.db` (if they exist), from the cache directory to the current directory.
