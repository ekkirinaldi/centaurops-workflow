# Run CentaurOps from a New Google Cloud Project

This guide will help you set up a CentaurOps development VM in a Google Cloud Platform project using Google Cloud Shell.
...
This script sets up a Debian-based VM with the CentaurOps package, Nginx, and the necessary configurations to run the CentaurOps Dev environment.
...
When running as a [spot (preemptible) instance](https://cloud.google.com/compute/docs/instances/preemptible), the code and VM will behave the same way as in a regular instance, executing the startup script to configure the environment, install necessary dependencies, and run the CentaurOps application. However, **due to the nature of spot instances, the VM may be terminated at any time if Google Cloud needs to reclaim the resources**. This makes spot instances suitable for fault-tolerant, stateless, or interruptible workloads that can handle unexpected terminations and restarts.
